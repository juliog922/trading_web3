...
impl Blockchain {
    ...
    pub fn get_difficulty(&mut self) -> u32 {
        ...
        if last_block.id % DIFFICULTY_ADJUSTMENT_INTERVAL_BLOCKS == 0 && last_block.id != 0 {
            ...
            let prev_difficulty_block = &self.chain[self.chain.len() -1 - DIFFICULTY_ADJUSTMENT_INTERVAL_BLOCKS];
            let time_taken = last_block.timestamp - prev_difficulty_block.timestamp;
            let time_expected = DIFFICULTY_ADJUSTMENT_INTERVAL_BLOCKS * BLOCK_GENERATION_INTERVAL_SECONDS;
    ...
    pub fn mine_block_by_stake(&mut self) -> Option<Block> {
        ...
        if Self::is_staking_valid(balance, difficulty, timestamp, &previous_hash, &address) {
    ...
    pub fn is_staking_valid(
        ...
        let decimal_staking_hash = BigUint::parse_bytes(&sha256_hash.as_bytes(), 16)
            .expect("Decimal staking Error");
    ...
    pub fn is_valid_block(&mut self, block: Block) -> bool {
            ...
            )
        {
            ...
            return false;
        } else if prev_block.id != block.id {
            warn!(
    ...
    pub fn get_leader(&mut self, block: Block) -> String {
